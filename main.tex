...
\chapter{Deep Learning Overview}
    ...
    \begin{center}
        ...
        \captionof{figure}{CNN for Computer Vision}
    \end{center}
    \newpage
...
\chapter{Used Solution: YOLOv8}
    ...
    \hline
    \textbf{Subset} & \textbf{Total Images} & \textbf{Basal Cell Carcinoma} & \textbf{Benign Keratosis} & \textbf{Melanoma} & \textbf{Nevus} \\
    \hline
    Train (Before Aug.) & 9,053  & 2,509 & 1,560 & 2,535 & 2,449 \\
    Train (After Aug.)  & 63,371 & 17,563 & 10,920 & 17,745 & 17,143 \\
    Validation          & 2,587  & 717   & 446   & 725   & 699 \\
    Test                & 1,293  & 358   & 223   & 362   & 350 \\
    \hline
    \end{tabular}%
    }
    \end{table}
    ...
    % \end{center}
    \lstinputlisting[
        caption={Inference command for YOLOv8 classification model},
        language=python,
        backgroundcolor=\color{backcolour},
        basicstyle=\ttfamily\color{black},
        keywordstyle=\color{blue},
    ...
    \begin{center}
        ...
        \captionof{figure}{Sample prediction with a confidence score of 0.54. The model incorrectly classified the image, and the moderate confidence score reflects a degree of uncertainty. Such cases often arise when the lesion shares visual features with other classes, making it more challenging for the model to distinguish accurately.}
    \end{center}
    
    \begin{center}
        \includegraphics[width=0.8\textwidth]{Pics/pred2.png}
        \captionof{figure}{Sample prediction with a confidence score of 1.00. In this example, the model produced a highly confident prediction, indicating strong certainty in the extracted features and their correspondence with the predicted class.}
    \end{center}
    ...
    %     \captionof{figure}{Export command used to convert the YOLOv8 model to PyTorch format.}
    % \end{center}

    \lstinputlisting[
        caption={Export command for YOLOv8 model to PyTorch format},
        language=python,
        backgroundcolor=\color{backcolour},
        basicstyle=\ttfamily\color{black},
        keywordstyle=\color{blue},
    ...
    % The exported model can be loaded directly into the GUI application for inference, allowing users to classify dermoscopic images without needing to retrain or modify the model.
    % \subsection*{Confusion Matrix}
    % \begin{center}
    ...
    \lstinputlisting[
        caption={Implementation of the Tkinter-based desktop GUI for skin lesion classification},
        language=python,
        backgroundcolor=\color{backcolour},
        basicstyle=\ttfamily\color{black},
        keywordstyle=\color{blue},
...
\bibitem[Ultralytics24]{ultralytics24}
